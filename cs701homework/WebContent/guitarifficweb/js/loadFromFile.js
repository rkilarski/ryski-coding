...
loadFromFile = {
	url : 'res/chords.xml',
	/**
	 * XMLHttpRequest - asynchronous loading of XML data from the XML file.
	 */
			...
			complete : function(xhr, result) {
				...
				$(response).find('chord').each(
					function() {
						...
						var chordPosition = $(this).find('chordPosition')
								.text();
						var chordFingering = $(this).find('chordFingering')
								.text();
						var chordFrets = $(this).find('chordFrets').text();
						var isLeftHanded = ($(this).find('isLeftHanded').text() == 'TRUE') ? true : false;
