 ...
 * 
 * <p>
 * A JTextPane is used as the underlying component in order to implement the styles. The styles are configurable and are presently used to effect bold
 * and italic styling on the text in the text area but can also be used to change font color, font size and font type.
 * 
 * <p>
 * This class uses a toolbar with buttons that are used to mark specific lines with the appropriate style. The toolbar can be dislodged from the
 * application to suit the users preference while maintaining its functionality.
 * 
...
public class StyledTextArea extends BaseTextArea
{
	...
	/**
	 * Initialized the styled text area and it's underlying styled document model.
	 */
	...
	/**
	 * Gets the panel to be placed into the applications's view containing the styled text area. The panel also contains the toolbar with the buttons
	 * that mark lines with the requisite style information denoting line type.
	 * 
		...
		lyric.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				try
				{
					...
					styledDocument.setCharacterAttributes(textArea.getSelectionStart(), textArea.getSelectionEnd() - textArea.getSelectionStart(), textArea.getStyle("Lyric"), true);
		...
		label.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				try
				{
					...
					styledDocument.setCharacterAttributes(textArea.getSelectionStart(), textArea.getSelectionEnd() - textArea.getSelectionStart(), textArea.getStyle("Label"), true);
				} catch (BadLocationException ble)
				{
				}
			}
		...
		chord.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				try
				{
					...
					styledDocument.setCharacterAttributes(textArea.getSelectionStart(), textArea.getSelectionEnd() - textArea.getSelectionStart(), textArea.getStyle("Chord"), true);
				} catch (BadLocationException ble)
				{
				}
			}
	...
	/**
	 * Gets the <code>JTextPane</code> associated with the text area. This method does not return the toolbar associated with the left pane of the
	 * application.
	 * 
	...
	/**
	 * Sets the internal model for the text area to the given string. This method is used to populate the text area during serialization or
	 * initialization.
	 * 
	 * @param textArea
	 *            the <code>JTextPane</code> created externally that is used to populate the internal model.
	 */
	...
	/**
	 * Returns the <code>StyledDocument</code> model used internally to serialize/deserialize the text pane content in the application.
	 * 
	 * @return the <code>StyledDocument</code> used internally to model the text in the text area.
	 */
	 ...
	 *            the document model created for this class to maintain the text in the text area.
	 */
